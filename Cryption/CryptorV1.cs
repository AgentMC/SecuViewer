...
namespace SecuViewer.Cryption
{
    internal class CryptorV1:ICryptor
    {
        ...
        public void Encrypt(CryptoData data, string what, Stream writer)
        {
            ...
            var totalPadding = (byte) (((garbageCount + 5) ^ data.Next()) & 0xFF);
        ...
        public bool CanDecrypt(Stream reader)
        {
            ...
            var read = reader.Read(buffer, 0, 4);
            if (read == 4 && buffer.SequenceEqual(_magic)) return true;
            reader.Position = 0;
            return false;
        }
    }
